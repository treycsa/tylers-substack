"""`substack` command: the daily loop, and each stage on its own.
    ...
    substack scan      # posts.jsonl -> candidates.json          (cron, 11 PM)
    ...
    substack publish   # manual (clipboard) or --browser
...
def cmd_scan(s: Settings, day: str) -> int:
    ...
    _dump(_work(s, day) / "candidates.json", {"scanned_posts": len(new_posts), "candidates": kept})
    print(f"scan: {len(new_posts)} new posts, {len(cands)} repos, {len(kept)} candidates after dedupe")
    return 0
...
def cmd_enrich(s: Settings, day: str) -> int:
    ...
    _dump(_work(s, day) / "repos.json", {"scanned_posts": c["scanned_posts"], "unique_repos": len(c["candidates"]),
                                        "repos": repos})
...
def cmd_score(s: Settings, day: str) -> int:
    ...
    _dump(_work(s, day) / "scored.json", {**{k: d[k] for k in ("scanned_posts", "unique_repos")}, "reviews": ranked})
    for r in ranked:
...
def cmd_test(s: Settings, day: str) -> int:
    ...
    _dump(_work(s, day) / "tested.json", {**{k: d[k] for k in ("scanned_posts", "unique_repos")}, "reviews": reviews})
    return 0
...
def cmd_review(s: Settings, day: str, auto: bool) -> int:
    ...
    _dump(_work(s, day) / "picks.json", {**{k: d[k] for k in ("scanned_posts", "unique_repos")}, "reviews": picks})
...
def cmd_build(s: Settings, day: str) -> int:
    ...
    issue = Issue(date=day, scanned_posts=d["scanned_posts"], unique_repos=d["unique_repos"], reviews=reviews,
...
def cmd_publish(s: Settings, day: str, browser: bool) -> int:
    meta = _load(_work(s, day) / "meta.json")
    handle = s.rubric["publication"].get("subdomain") or s.rubric["publication"]["handle"]
    if browser:
...
def main(argv: list[str] | None = None) -> int:
    ...
    for name in ("scan", "enrich", "score", "test", "build"):
        sub.add_parser(name)
    sub.add_parser("review").add_argument("--auto", action="store_true", help="no human step: WATCHING only")
    sub.add_parser("publish").add_argument("--browser", action="store_true")
    ...
    if a.cmd == "publish":
        return cmd_publish(s, day, a.browser)
    if a.cmd == "login":
    ...
    if a.cmd == "daily":
        for fn in (cmd_scan, cmd_enrich, cmd_score, cmd_test):
            if fn(s, day):
                return 1
        cmd_review(s, day, a.auto)
        cmd_build(s, day)
        if a.publish:
            cmd_publish(s, day, browser=False)
        return 0
    return 2
